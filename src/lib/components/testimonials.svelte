...
<div class="flex justify-center items-center min-h-screen">
    <figure class="max-w-screen-md bg-white text-black">
        ...
        <blockquote>
            <p class="text-2xl font-semibold" style:opacity={$opacity}>
                "{testimonials[currentIndex].text}"
            </p>
        </blockquote>
        <figcaption class="flex items-center mt-1 space-x-3 rtl:space-x-reverse">
            <img class="w-6 h-6 rounded-full" src={testimonials[currentIndex].image} alt="profile picture">
            <div class="flex items-center divide-x-2 rtl:divide-x-reverse divide-gray-300">
                <cite class="pe-3 font-medium">{testimonials[currentIndex].name}</cite>
                <cite class="ps-3 text-sm text-gray-500">{testimonials[currentIndex].position}</cite>
            </div>
        </figcaption>
    </figure>
</div>
